...
/// <summary>
/// Detects the local Claude Code installation and reads non-secret account / plan
/// metadata from <c>%USERPROFILE%\.claude.json</c> (the <c>oauthAccount</c> block).
/// The live usage snapshot lives in <see cref="CacheFilePath"/>.
///
/// The secret token file (<c>%USERPROFILE%\.claude\.credentials.json</c>) is never
/// read: Claude Code keeps <c>usage_cache.json</c> fresh while it runs, so no
/// authenticated API call is required.
/// </summary>
public sealed class ClaudeAuth
{
    ...
    /// <summary>Plan/identity metadata, written by Claude Code at sign-in.</summary>
    public static string SettingsFilePath => Path.Combine(HomePath, ".claude.json");

    /// <summary>Live usage cache (5h + weekly utilisation), refreshed by Claude Code.</summary>
    public static string CacheFilePath => Path.Combine(HomePath, ".claude", "usage_cache.json");

    /// <summary>True when Claude Code appears to be installed and signed in on this PC.</summary>
    public bool IsConfigured => File.Exists(SettingsFilePath);
